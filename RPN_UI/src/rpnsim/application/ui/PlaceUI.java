...
public class PlaceUI extends MovableNode {
	...
	public PlaceUI(RPN rpn) {
		super(rpn,rpn.addPlace(),DragContainer.DragPlace);
	...
	public PlaceUI(RPN rpn, Point2D position, ArrayList<String> tokenList) {
		
		// Prosthese ena neo place sto RPN mas
		super(rpn,rpn.addPlace(),DragContainer.DragPlace);
		...
		for(String tokenName: tokenList) {
			...
			grid.setVgap(2); 
			grid.setHgap(2);
			TokenUI token = new TokenUI(rpn,this);
	...
	public void refresh() {
		...
		if( rpn.getPlace(myNode.name) == null ) {
			delete();
		...
		for( TokenUI tokenUI : tokenArray ) {
			...
			Token token = rpn.getToken(tokenUI.myToken.name);
			if( token == null ) {
				removeToken(tokenUI);
				hasChanged = true;
			}
			...
			if( token.getPlace() != (Place)myNode ) {
				removeToken(tokenUI);
				hasChanged = true;
			}
		}
	...
	public void delete() {
		...
		for(int i=0; i<tokenArray.length; i++) {
			...
			rpn.removeToken(tokenUI.myToken);
	...
	*/
	...
	public void addToken(Token token) {
		...
		TokenUI tokenUI = new TokenUI(rpn, this, token);
	...
	public List<Pair<String,Object>> getDataList(){
		...
		for( TokenUI tokenUI : tokens ) 
			tokenList.add( new Pair<String,String>("token",tokenUI.myToken.name) );
