...
import rpnsim.application.model.Bond;
...
public class BondUI extends SelectableNode {
	...
	public BondUI(Bond existingBond) {
		super(existingBond.getSource().rpn);
		init();
		...
		this.A = rpn.scrollArea.findToken(tokenA);
		this.B = rpn.scrollArea.findToken(tokenB);
	...
	public BondUI(TokenUI tokenA, TokenUI tokenB) {
		...
		rpn.addBondFromRPN(A.myToken, B.myToken);
		
	}
	
	public BondUI(RPN rpn) {
		super(rpn);
		init();
	}
	...
	public void refresh() {
		...
		// Check if the RPN still has the bond A-B
		if( !rpn.hasBond( A.getName(), B.getName())) {
			remove();
			return;
		}
		...
		if( rpn.scrollArea != null ) {
			if(!rpn.scrollArea.hasNode(A) ||  !rpn.scrollArea.hasNode(B) ) {
				remove();
				return;
			}
		}
